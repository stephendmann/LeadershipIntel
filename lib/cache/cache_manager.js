...
/** 仅普通对象参与“空值”判定，避免误伤 Date / Map 等非普通对象 */
...
/**
 * 可用的缓存值。
 * Notion 拉取失败时 convertNotionToSiteData 会短路返回 {}，
 * 空对象若被写入缓存，会把一次瞬时故障固化成整个缓存周期的空白站点。
 */
    ...
    const promise = withFileLock(
      ...
      async () => {
        ...
        if (isUsableCacheValue(data)) {
          ...
          cacheLog('SKIP-SET', key, 'empty or invalid result, not cached')
        }
...
export async function setDataToCache(key, data, customCacheTime) {
  if (!isUsableCacheValue(data)) return
