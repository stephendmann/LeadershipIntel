...
#### 4. 添加自动化 Workflow（本 fork 未启用）

本 fork 未部署文档站，因此未包含自动化 workflow（此前的 `deploy-docs-site.yml` 因缺少上述 Secrets 持续失败，已移除）。如需启用，可参考上游 [`deploy-docs-site.yml`](https://github.com/notionnext-org/NotionNext/blob/main/.github/workflows/deploy-docs-site.yml) 重新添加到 `.github/workflows/`，效果：

- `push` 到 `main` 且变更 `docs/**`、`.vitepress/**` 等 → 自动 `yarn docs:site:build` 并部署  
- 也可在 GitHub **Actions** 页手动 **Run workflow**

添加后首次 push，到 **Actions** 查看 **Deploy docs site (VitePress)** 是否成功；未添加 workflow 前，见下方「本地开发与构建」手动部署。
